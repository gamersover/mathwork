...
class Chess:
    ...
    def __init__(self, chess_size, screen=None, print_info=True, FPS=60):
        ...
        if self.screen:
            ...
            self.radius_y = int(self.screen_size[0]/chess_size[1])
            self.radius_x = int(self.screen_size[1]/chess_size[0])
            self.radius = int(min(self.radius_x, self.radius_y)/4)
        ...
        for i in range(self.chess_size_x):
            for j in range(self.chess_size_y):
                if self.chess_matrix[i][j] == 1:
                    pygame.draw.circle(self.screen, BLACK, [int(self.radius_x/2) + i*self.radius_x, int(self.radius_y/2) + j*self.radius_y], self.radius, 2)
                elif self.chess_matrix[i][j] == 2:
                    pygame.draw.circle(self.screen, BLACK, [int(self.radius_x/2) + i*self.radius_x, int(self.radius_y/2) + j*self.radius_y], self.radius, 0)
        
        for i in range(1, self.chess_size_y):
            # print(self.chess_size_y)
            pygame.draw.line(self.screen, BLACK, (0, self.radius_y*i), (self.screen_size[0], self.radius_y*i), 1)

        for i in range(1, self.chess_size_x):
            # print(self.chess_size_x)
            pygame.draw.line(self.screen, BLACK, (self.radius_x*i, 0), (self.radius_x*i, self.screen_size[1]), 1)
    ...
    def game_start(self, start_position=None):
        ...
        if start_position:
            ...
            while True:
                start_y, start_x = map(int, input("please input your start position like 1 2:").split())
